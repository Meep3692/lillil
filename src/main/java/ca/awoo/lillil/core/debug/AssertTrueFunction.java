...
public class AssertTrueFunction extends SFunction {
    ...
    public SExpression apply(SExpression... args) throws LillilRuntimeException {
        if(args.length < 2){
            throw new LillilRuntimeException(this, "assert-true requires at least 2 arguments");
        }
        if(!args[0].isString()){
            throw new LillilRuntimeException(args[0], "assert-true requires a string argument");
        }
        for(int i = 1; i < args.length; i++){
            if(!args[i].isBoolean()){
                throw new LillilRuntimeException(args[i], "assert-true requires a boolean argument, got " + args[i].getClass().getSimpleName() + " instead");
            }
